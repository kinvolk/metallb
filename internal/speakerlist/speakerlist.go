...
func (sl *SpeakerList) Start(client *k8s.Client) {
	// TODO: the k8s client parameter ideally, should be a parameter to
	...
	// 3. The new Kubernetes client uses the just created controller to set the callbacks [3]
	//
	// Then, we have a loop if we add the kubernetes client to speakerlist.New():
	//
	// 1. To create a kubernetes client we need a controller
	// 2. To create a controller we need a speaker list
	// 3. To create a speakerlist we need a kubernetes client
	//
...
// mlJoin joins this memberlist cluster with other speaker pods that are not
// currently members of this cluster. In other words, it does a memberlist.Join()
// with the IPs in mlSpeakerIPs that are not members of the cluster.
func (sl *SpeakerList) mlJoin() {
	...
	for _, ip := range sl.mlSpeakerIPs {
		// If an IP is not member of the cluster, add it to joinIPs.
